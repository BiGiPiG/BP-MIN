...
public enum ProfileColor {
    ...
    private static final Map<String, ProfileColor> GRADIENT_MAP;
    ...
    public static ProfileColor fromGradient(String gradient) {
        if (gradient == null || gradient.isBlank()) {
            return null;
        }
        ProfileColor color = GRADIENT_MAP.get(gradient);
        if (color == null) {
